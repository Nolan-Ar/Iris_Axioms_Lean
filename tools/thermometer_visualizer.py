...
class RAD:
    ...
    def is_cold(self) -> bool:
        """r_t < 0.85 → System is cold (deflation)"""
        return self.thermometre() < 0.85

    def is_hot(self) -> bool:
        """r_t > 1.15 → System is hot (inflation)"""
        return self.thermometre() > 1.15

    def is_equilibrium(self) -> bool:
        """0.85 ≤ r_t ≤ 1.15 → System is in equilibrium"""
        r_t = self.thermometre()
        return 0.85 <= r_t <= 1.15
...
class IrisThermometerVisualizer:
    ...
    def simulate_evolution(self, rad_initial: RAD, cycles: int = 100,
                          adjustment_rate: float = 0.1) -> List[Tuple[int, RAD]]:
        """
        Simulate economic evolution with automatic η adjustment

        Args:
            rad_initial: Initial RAD state
            cycles: Number of cycles to simulate
            adjustment_rate: Rate of η adjustment (default 10%)
        ...
        for cycle in range(1, cycles + 1):
            ...
            # Ensure η bounds: 0 < η ≤ 2
            new_eta = max(0.1, min(2.0, new_eta))

            # Simulate value creation: ΔV = η × E (simplified)
            # Assume constant energy input E = 1000 per cycle
            E_per_cycle = 1000
            delta_V = new_eta * E_per_cycle
            delta_D = -delta_V  # Conservation: D mirrors V

            # Update RAD state
            rad_next = RAD(
                D_total=rad_current.D_total + delta_D,
                V_on_total=rad_current.V_on_total + delta_V,
                eta=new_eta,
                kappa=rad_current.kappa
            )
    ...
    def plot_evolution(self, history: List[Tuple[int, RAD]],
        ...
        # Plot 2: η evolution
        ax2.plot(cycles, eta_values, 'purple', linewidth=2, label='η')
        ax2.axhline(y=1.0, color='gray', linestyle='--', alpha=0.5, label='η = 1.0')
        ax2.set_xlabel('Cycle', fontsize=12)
        ax2.set_ylabel('η (Value creation efficiency)', fontsize=12)
        ax2.set_title('η Automatic Adjustment', fontsize=14, fontweight='bold')
        ax2.legend()
        ...
        ax4.set_title('Total Debt Evolution', fontsize=14, fontweight='bold')
        ax4.legend()
    ...
    def print_summary(self, history: List[Tuple[int, RAD]]):
...
def main():
    ...
    if args.simulate:
        ...
        print(f"  Status: ", end='')
        if rad.is_cold():
            print("COLD - System needs more value creation (increase η)")
        elif rad.is_hot():
            print("HOT - System has too much debt (decrease η)")
        else:
